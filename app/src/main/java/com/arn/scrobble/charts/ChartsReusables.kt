...
import androidx.compose.material3.SplitButtonDefaults.OutlinedLeadingButton
import androidx.compose.material3.SplitButtonDefaults.OutlinedTrailingButton
...
private val TimePeriodType.stringRes
    get() = when (this) {
        TimePeriodType.CONTINUOUS -> R.string.charts_continuous
        TimePeriodType.CUSTOM -> R.string.charts_custom
        TimePeriodType.WEEK -> R.string.weeks
        TimePeriodType.MONTH -> R.string.months
        TimePeriodType.YEAR -> R.string.years
        TimePeriodType.LISTENBRAINZ -> R.string.listenbrainz
...
fun getPeriodTypePluralRes(periodType: TimePeriodType): Int {
    return when (periodType) {
        TimePeriodType.WEEK -> R.plurals.num_weeks
        TimePeriodType.MONTH -> R.plurals.num_months
        TimePeriodType.YEAR -> R.plurals.num_years
...
fun TimePeriodSelector(
    ...
    val isListenBrainz by Scrobblables.current.mapLatest { it?.userAccount?.type == AccountType.LISTENBRAINZ }
    ...
    LaunchedEffect(selectedPeriod) {
        ...
        selectedPeriod?.let { selectedPeriod ->
            ...
            if (periodType != TimePeriodType.CONTINUOUS) {
                ...
                if (selectedPeriod.period != null && selectedPeriod.period != Period.OVERALL) {
                    val cal = Calendar.getInstance()
                    cal.setUserFirstDayOfWeek()
                    cal.setMidnight()
                    val duration = selectedPeriod.period.toDuration(endTime = cal.timeInMillis)
                    prevPeriod =
                        selectedPeriod.period.toTimePeriod(endTime = cal.timeInMillis - duration)
                }
            }
    ...
    Row(
                        ...
                        if (periodType == TimePeriodType.WEEK || periodType == TimePeriodType.MONTH) {
                            OutlinedTrailingButton(
                                ...
                                Icon(
                                    ...
                                    contentDescription = stringResource(id = R.string.appwidget_period)
                                )
                            }
                        }
                    },
        ...
        LazyRow(
            ...
            contentPadding = panoContentPadding(bottom = false)
        ) {
