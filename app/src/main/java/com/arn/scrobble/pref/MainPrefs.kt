...
class MainPrefs(context: Context): Krate {
    ...
    var allowedArtists by stringSetPref(PREF_ALLOWED_ARTISTS, setOf())
    var lastfmDisabled by booleanPref(PREF_LASTFM_DISABLE, false)
    var submitNowPlaying by booleanPref(PREF_NOW_PLAYING, true)
    var pixelNowPlaying by booleanPref(PREF_PIXEL_NP, true)
    var fetchAlbumArtist by booleanPref(PREF_FETCH_AA, false)
    ...
    var themeRandom by booleanPref(PREF_THEME_RANDOM, false)
    var themeSameTone by booleanPref(PREF_THEME_SAME_TONE, false)
    var themePaletteBackground by booleanPref(PREF_THEME_PALETTE_BG, true)
    var firstRun by booleanPref(PREF_ACTIVITY_FIRST_RUN, true)
    var heroGraphDetails by booleanPref(PREF_ACTIVITY_GRAPH_DETAILS, true)
    var lastHomePagerTab by intPref(PREF_ACTIVITY_LAST_TAB, 0)
    var lastChartsPeriod by intPref(PREF_ACTIVITY_LAST_CHARTS_PERIOD, 1)
    var lastChartsWeekTo by longPref(PREF_ACTIVITY_LAST_CHARTS_WEEK_TO, 0)
    var lastChartsWeekFrom by longPref(PREF_ACTIVITY_LAST_CHARTS_WEEK_FROM, 0)
    var scrobblesTodayCached by intPref(PREF_ACTIVITY_TODAY_SCROBBLES, 0)
    var scrobblesTotalCached by intPref(PREF_ACTIVITY_TOTAL_SCROBBLES, 0)
    var scrobblingSince by longPref(PREF_ACTIVITY_SCROBBLING_SINCE, 0)
    var lastRandomType by intPref(PREF_ACTIVITY_LAST_RANDOM_TYPE, Stuff.TYPE_TRACKS)
    var profilePicUrlCached by stringPref(PREF_ACTIVITY_PROFILE_PIC)
    var userTopTagsFetched by booleanPref(PREF_ACTIVITY_USER_TOP_TAGS_FETCHED, false)
    var scrubLearnt by booleanPref(PREF_ACTIVITY_SCRUB_LEARNT, false)
    var longPressLearnt by booleanPref(PREF_ACTIVITY_LONG_PRESS_LEARNT, false)
    var regexEditsLearnt by booleanPref(PREF_ACTIVITY_REGEX_EDITS_LEARNT, false)

    var searchHistory by stringSetPref(PREF_ACTIVITY_SEARCH_HISTORY)
    var tagHistory by stringSetPref(PREF_ACTIVITY_TAG_HISTORY)

    var notiLockscreen by booleanPref(PREF_LOCKSCREEN_NOTI, false)
    var notiScrobbling by booleanPref(CHANNEL_NOTI_SCROBBLING, true)
    var notiError by booleanPref(CHANNEL_NOTI_SCR_ERR, true)
    var notiWeeklyDigests by booleanPref(CHANNEL_NOTI_DIGEST_WEEKLY, true)
    var notiMonthlyDigests by booleanPref(CHANNEL_NOTI_DIGEST_MONTHLY, true)
    var notiPendingScrobbles by booleanPref(CHANNEL_NOTI_PENDING, true)
    var notiNewApp by booleanPref(CHANNEL_NOTI_NEW_APP, true)
    ...
    var customListenbrainzRoot by stringPref(PREF_LB_CUSTOM_ROOT, Stuff.LISTENBRAINZ_API_ROOT)
    var gnufmUsername by stringPref(PREF_GNUFM_USERNAME)
    var gnufmSessKey by stringPref(PREF_GNUFM_SESS_KEY)
    var gnufmRoot by stringPref(PREF_GNUFM_ROOT, "https://")
    ...
    var proStatus by booleanPref(PREF_PRO_STATUS, false)
    var proSkuJson by stringPref(PREF_PRO_SKU_JSON)
    var digestSeconds by intPref(PREF_DIGEST_SECONDS) //nullable
    var scrobbleCount by intPref(PREF_SCROBBLE_COUNT, 0)
    var firstLaunchTime by longPref(PREF_FIRST_LAUNCHED) //nullable
    var dontAskForRating by booleanPref(PREF_DONT_SHOW_AGAIN, false)
    var prefVersion by intPref(PREF_VERSION, 0)
    ...
    companion object {
        ...
        const val PREF_DONT_SHOW_AGAIN = "dontshowagain"
