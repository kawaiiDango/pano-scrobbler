...
interface CachedArtistsDao {
    ...
    suspend fun find(term: String, limit: Int = 50): List<CachedArtist>

    @Query("SELECT * FROM $tableName WHERE artistName like :artistName LIMIT 1")
    suspend fun findExact(artistName: String): CachedArtist?

    @Query("SELECT count(1) FROM $tableName")
    suspend fun count(): Int

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(entry: List<CachedArtist>)

    @Update(onConflict = OnConflictStrategy.REPLACE)
    suspend fun update(entry: CachedArtist)

    @Delete
    suspend fun delete(entry: CachedArtist)
    ...
    companion object {
        ...
        suspend fun CachedArtistsDao.deltaUpdate(
            artist: CachedArtist,
            deltaCount: Int,
